// #3 Quiz - Set of 30 questions with SY0-701 objective codes (Fixed)
export const thirdQuiz = [
  {
    question: "Penetration tests should ideally be performed in which type of environment to minimize business risk?",
    options: ["Production environment", "Testing environment", "Hybrid environment", "Live user environment"],
    answer: "Testing environment",
    objective: "Domain 4.5 – Security Assessment (Penetration Testing)"
  },
  {
    question: "What is a CRL (Certificate Revocation List)?",
    options: ["A list of invalidated certificates", "A list of trusted certificates", "A list of expired certificates", "A list of pending certificates"],
    answer: "A list of invalidated certificates",
    objective: "Domain 1.4 – Cryptography (PKI & Certificates)"
  },
  {
    question: "A Certificate Authority (CA) is responsible for deploying and managing digital certificates.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 1.4 – Cryptography (PKI & Certificates)"
  },
  {
    question: "CSR stands for Certificate Signing Request and is used to send a public key to a certificate authority.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 1.4 – Cryptography (PKI & Certificates)"
  },
  {
    question: "Which of the following security measures help protect mobile devices?",
    options: ["Credential stuffing and port scanning", "Full disk encryption and biometric authentication", "SQL injection and XSS protection", "DNS poisoning and ARP spoofing"],
    answer: "Full disk encryption and biometric authentication",
    objective: "Domain 4.1 – Mobile Security (Device Security)"
  },
  {
    question: "What is the correct order of incident response operations?",
    options: [
      "Preparation, Detection, Analysis, Containment, Eradication, Recovery, Lessons Learned",
      "Detection, Containment, Analysis, Lessons Learned, Recovery, Preparation, Eradication",
      "Analysis, Detection, Preparation, Containment, Recovery, Eradication, Lessons Learned",
      "Containment, Detection, Analysis, Preparation, Eradication, Lessons Learned, Recovery"
    ],
    answer: "Preparation, Detection, Analysis, Containment, Eradication, Recovery, Lessons Learned",
    objective: "Domain 4.8 – Incident Response"
  },
  {
    question: "SPF (Sender Policy Framework) is used for email server authentication.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 4.6 – Identity & Access Management (Email Security)"
  },
  {
    question: "An 802.1Q trunk link can carry traffic for multiple VLANs.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 3.1 – Network Security (Network Design & Segmentation)"
  },
  {
    question: "Server hardening guides are available for most applications and services.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 4.1 – Endpoint Security (Configuration & Hardening)"
  },
  {
    question: "What type of control is an Intrusion Prevention System (IPS)?",
    options: ["Compensating", "Preventive", "Directive", "Deterrent"],
    answer: "Preventive",
    objective: "Domain 3.1 – Network Security (Security Devices)"
  },
  {
    question: "Which reconnaissance technique typically requires more direct interaction with the target system?",
    options: ["OSINT (Open Source Intelligence)", "Active Reconnaissance", "Passive Reconnaissance", "Social Media Analysis"],
    answer: "Active Reconnaissance",
    objective: "Domain 2.1 – Threat Actors & Motivations (Reconnaissance)"
  },
  {
    question: "A 'partially known environment' describes when an attacker has some knowledge about the system being tested.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 4.5 – Security Assessment (Penetration Testing)"
  },
  {
    question: "Exfiltration refers to unauthorized data transfer from a system.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 2.4 – Network Attacks (Data Exfiltration)"
  },
  {
    question: "Typosquatting is when attackers register misspelled domain names to trick users.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 2.2 – Social Engineering (Typosquatting)"
  },
  {
    question: "VM escape is a vulnerability that allows communication between a virtual machine and the host or other VMs.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 2.3 – Vulnerabilities (Virtualization)"
  },
  {
    question: "SDN stands for Software Defined Networking.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 3.1 – Network Security (Network Design)"
  },
  {
    question: "An Amplified DDoS attack increases traffic volume by exploiting services that return large responses.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 2.4 – Network Attacks (DDoS)"
  },
  {
    question: "In network security, 'accounting' refers to tracking user sessions, login/logout times, and resource usage.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 4.6 – Identity & Access Management (AAA Concepts)"
  },
  {
    question: "What does PCI-DSS stand for?",
    options: ["Payment Card Industry Data Security Standard", "Protocol Control Industry Data Security Standard", "Private Card Information Data Security System", "Public Card Industry Digital Security Standard"],
    answer: "Payment Card Industry Data Security Standard",
    objective: "Domain 5.3 – Security Compliance (Standards & Regulations)"
  },
  {
    question: "Can a firewall operate alongside a VPN concentrator?",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 3.1 – Network Security (Security Devices)"
  },
  {
    question: "Does a VPN concentrator typically rely on a central AAA server for user authentication?",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 3.1 – Network Security (VPNs)"
  },
  {
    question: "What does AAA stand for in network security?",
    options: ["Authentication, Authorization, Accounting", "Authentication, Authorization, Attestation", "Access, Authentication, Auditing", "Authorization, Accounting, Auditing"],
    answer: "Authentication, Authorization, Accounting",
    objective: "Domain 4.6 – Identity & Access Management (AAA Concepts)"
  },
  {
    question: "In security, benchmarks refer to best-practice guidelines for system configuration.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 4.1 – Endpoint Security (Configuration & Hardening)"
  },
  {
    question: "Allowing personal mobile devices on a secure corporate network increases the risk of intellectual property theft.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 4.1 – Mobile Security (BYOD Risks)"
  },
  {
    question: "A cold site is a disaster recovery location with minimal setup and infrastructure.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 3.4 – Business Continuity & Disaster Recovery"
  },
  {
    question: "RBAC stands for Role-Based Access Control, used for managing user rights in systems.",
    options: ["True", "False"],
    answer: "True",
    objective: "Domain 4.6 – Identity & Access Management (Access Controls)"
  },
  {
    question: "What is the main benefit of Software Defined Networking (SDN)?",
    options: [
      "It allows centralized control by separating the control and data planes.",
      "It makes network devices autonomous by integrating control with the data plane.",
      "It eliminates the need for physical network infrastructure.",
      "It provides automatic network security without configuration."
    ],
    answer: "It allows centralized control by separating the control and data planes.",
    objective: "Domain 3.1 – Network Security (Network Design)"
  },
  {
    question: "What does MSP stand for?",
    options: ["Managed Security Provider", "Managed Service Provider", "Multi-Service Platform", "Master Security Protocol"],
    answer: "Managed Service Provider",
    objective: "Domain 5.4 – Third-Party Risk Management (Managed Services)"
  },
  {
    question: "What is an MSP?",
    options: [
      "A third-party company that remotely manages a customer's IT infrastructure or end-user systems.",
      "A local department that internally manages an organization's IT support.",
      "A software platform for managing multiple security services.",
      "A certification program for security professionals."
    ],
    answer: "A third-party company that remotely manages a customer's IT infrastructure or end-user systems.",
    objective: "Domain 5.4 – Third-Party Risk Management (Managed Services)"
  },
  {
    question: "Is a Jump Server used in network security?",
    options: ["Yes", "No", "Only in cloud environments", "Only for external access"],
    answer: "Yes",
    objective: "Domain 3.1 – Network Security (Access Controls / Jump Servers)"
  }
];

//Old
// #3 Quiz - Set of 30 questions
// export const thirdQuiz = [  {
// question: "Penetration tests are designed to expose and exploit system vulnerabilities and should run in what type of environment?",
// options: [
// "Testing environment",
// "Production environment"
// ],
// answer: "Testing environment"
// },
// {
// question: "What is a CRL (Certificate Revocation List)?",
// options: [
// "A list of trusted certificates",
// "A list of invalidated certificates"
// ],
// answer: "A list of invalidated certificates"
// },
// {
// question: "A Certificate Authority (CA) is responsible for deploying and managing digital certificates.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "CSR stands for Certificate Signing Request and is used to send a public key to a certificate authority.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "Which of the following security measures help protect mobile devices?",
// options: [
// "Full disk encryption and biometric authentication",
// "Credential stuffing and port scanning"
// ],
// answer: "Full disk encryption and biometric authentication"
// },
// {
// question: "What is the correct order of incident response operations?",
// options: [
// "Preparation, Detection, Analysis, Containment, Eradication, Recovery, Lessons Learned",
// "Detection, Containment, Analysis, Lessons Learned, Recovery, Preparation, Eradication"
// ],
// answer: "Preparation, Detection, Analysis, Containment, Eradication, Recovery, Lessons Learned"
// },
// {
// question: "SPF (Sender Policy Framework) is used for email server authentication.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "An 802.1Q trunk link can carry traffic for multiple VLANs.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "Server hardening guides are available for most applications and services.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "What type of control is an Intrusion Prevention System (IPS)?",
// options: ["Compensating", "Directive", "Deterrent", "Detective"],
// answer: "Detective"
// },
// {
// question: "Which of the following is more commonly associated with malicious intent?",
// options: ["Passive Reconnaissance", "OSINT (Open Source Intelligence)"],
// answer: "Passive Reconnaissance"
// },
// {
// question: "A 'partially known environment' describes when an attacker has some knowledge about the system being tested.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "Exfiltration refers to unauthorized data transfer from a system.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "Typosquatting is when attackers register misspelled domain names to trick users.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "VM escape is a vulnerability that allows communication between a virtual machine and the host or other VMs.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "SDN stands for Software Defined Networking.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "An Amplified DDoS attack increases traffic volume by exploiting services that return large responses.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "In network security, 'accounting' refers to tracking user sessions, login/logout times, and resource usage.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "What does PCI-DSS stand for?",
// options: [
// "Payment Card Industry Data Security Standard",
// "Protocol Control Industry Data Security Standard"
// ],
// answer: "Payment Card Industry Data Security Standard"
// },
// {
// question: "Can a firewall operate alongside a VPN concentrator?",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "Does a VPN concentrator rely on a central AAA server for user authentication?",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "What does AAA stand for in network security?",
// options: [
// "Authentication, Authorization, Accounting",
// "Authentication, Authorization, Attestation"
// ],
// answer: "Authentication, Authorization, Accounting"
// },
// {
// question: "In security, benchmarks refer to best-practice guidelines for system configuration.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "Allowing personal mobile devices on a secure corporate network increases the risk of intellectual property theft.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "A cold site is a disaster recovery location with minimal setup and infrastructure.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "RBAC stands for Role-Based Access Control, used for managing user rights in systems.",
// options: ["True", "False"],
// answer: "True"
// },
// {
// question: "What is the main benefit of Software Defined Networking (SDN)?",
// options: [
// "It allows centralized control by separating the control and data planes.",
// "It makes network devices autonomous by integrating control with the data plane."
// ],
// answer: "It allows centralized control by separating the control and data planes."
// },
// {
// question: "What does MSP stand for?",
// options: ["Managed Service Provider", "Managed Security Provider"],
// answer: "Managed Service Provider"
// },
// {
// question: "What is an MSP?",
// options: [
// "A third-party company that remotely manages a customer's IT infrastructure or end-user systems.",
// "A local department that internally manages an organization's IT support."
// ],
// answer: "A third-party company that remotely manages a customer's IT infrastructure or end-user systems."
// },

// {
// question: "Is a Jump Server used in network security?",
// options: ["Yes", "No"],
// answer: "Yes"
// }];